...
from src.core.dependencies import get_log_service, TenantContextDep
...
async def get_llm_logs(
    ...
    """
    Get LLM logs with pagination and optional endpoint filtering for the current tenant.
    ...
    """
    try:
        logs = await log_service.get_logs_paginated_by_tenant(page, per_page, endpoint, tenant_context)
...
async def count_llm_logs(
    ...
    """
    Count LLM logs with optional endpoint filtering for the current tenant.
    ...
    """
    try:
        return await log_service.count_logs_by_tenant(endpoint, tenant_context)
    except Exception as e:
...
async def get_unique_endpoints(
    ...
    """
    Get list of unique endpoints in the logs for the current tenant.
    ...
    """
    try:
        return await log_service.get_unique_endpoints_by_tenant(tenant_context)
    except Exception as e:
...
async def get_log_stats(
    ...
    """
    Get statistics about LLM usage for the current tenant.
    ...
    """
    try:
        return await log_service.get_log_stats_by_tenant(days, tenant_context)
    except Exception as e:
